...
@author:
    ...
    Updated to account for additional and missing data in additional conditions.
...
conditionNames = ['split50', 'split25_lower', 'split25_upper']
conditionLabels = ['Mid Split', 'Lower Split', 'Upper Split']
...
#Export group data
groupData.fillna('').to_csv(homeDir+'\\..\\Results\\Processed\\groupData_allConditions.csv',
                            index = False)
...
#Group and describe just moment arm data
describedData_ma = groupData.drop(['lineOfAction', 'stabilityRatio'],
                                   axis = 1).groupby(['region',
                                                      'condition',
                                                      'position',
                                                      'load',
...
#Group and describe just stability ratio data
describedData_sr = groupData.drop(['lineOfAction', 'momentArm'],
                                   axis = 1).groupby(['region',
                                                      'condition',
                                                      'position',
                                                      'load',
...
#Export summary data
describedData_loa.to_csv('..\\Results\\Processed\\summaryLoA_groupData_allConditions.csv',
                         index = True)
describedData_ma.to_csv('..\\Results\\Processed\\summaryMomentArm_groupData_allConditions.csv',
                        index = True)
describedData_sr.to_csv('..\\Results\\Processed\\summaryStabilityRatio_groupData_allConditions.csv',
                        index = True)
...
for subscap in subscapNames:
    ...
    #Subscap points
    ax.scatter(ssPts_x, ssPts_y, s = 15, c = 'red', zorder = 5)

    #Display fits to these points
    #Fit line
    m,c = np.polyfit(ssPts_x, ssPts_y, 1)
    #Plot fitted line
    ...
    # #Extend subscapularis line
    # #Line
    # ax.plot(np.array(((ax.get_xlim()[1] / 5), np.max(ssPts_x))),
